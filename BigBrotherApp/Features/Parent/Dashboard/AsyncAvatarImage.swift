    ...
    var body: some View {
        ...
        .task(id: base64) {
            ...
            if let hit = AvatarImageCache.cached(for: base64) {
                BBLog("[AvatarCache] HIT at mount (src=\(base64.count) bytes)")
                image = hit
                return
            }
            BBLog("[AvatarCache] MISS at mount — pre-warm didn't beat view, decoding now (src=\(base64.count) bytes)")
            let t0 = Date()
            let decoded = await AvatarImageCache.image(for: base64)
            let ms = Int(Date().timeIntervalSince(t0) * 1000)
            BBLog("[AvatarCache] MISS resolved in \(ms)ms")
            await MainActor.run {
                image = decoded
            }
        }
